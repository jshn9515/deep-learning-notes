...
## 8.10.6 Why Decoder-Only Is So Popular Today
...
At the same time, decoder-only models also have a unified inference form: give a prompt and continue generating. Many tasks can be written as prompt completion: question answering gives a question and generates an answer; translation gives an instruction and source sentence and generates the target sentence; classification gives text and label descriptions and generates the label; code tasks give requirements or a prefix and generate code; dialogue gives chat history and generates the next response. In other words, decoder-only models unify many tasks under the same interface:
...
## 8.10.7 When to Use Which Structure

If the task is mainly to understand an existing input and output a class, label, or vector representation, encoder-only is usually very natural. For example:
...
If the task is open-ended generation, decoder-only is usually very natural. For example:
...
If the task is a clear input-to-output conversion, encoder-decoder is usually very natural. For example:
...
In this section, we compared the three common Transformer architectures.
