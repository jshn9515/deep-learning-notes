...
title: "11.4 Embedding、LM Head 与 Weight Tying"
order: 4
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch11-gpt2-from-scratch/ch11.4-weight-tying.ipynb){fig-align="left"}

前面几节里，我们已经完成了三件事：

- 11.1 讲清楚了语言模型的目标：给定前缀，预测下一个 token；
- 11.2 把 GPT block 组装成了 MiniGPT；
- 11.3 解释了 tokenizer 如何把文本变成 token ids，以及 `vocab_size` 从哪里来。
...
## 11.4.2 Token Embedding：不是把数字大小喂给模型
...
## 11.4.3 Positional Embedding：位置不是词表的一部分

在 11.2 里，我们把 token embedding 和 positional embedding 加在一起：
...
## 11.4.4 LM Head：把 hidden state 变成词表打分
...
## 11.4.5 Weight Tying：输入和输出共享同一张词表向量表
...
## 11.4.8 把 Weight Tying 放进 MiniGPT
