...
title: "4.2 Momentum 与 Nesterov Momentum"
order: 2
---
...
这一节我们要看的 momentum，就是为了解决这个问题提出的一个非常自然的想法：
...
假设损失函数的形状像一个狭长的山谷。沿着 $\theta_1$ 方向，损失变化很快；沿着 $\theta_2$ 方向，损失变化很慢。这样的损失曲面在神经网络训练中并不少见。
...
再定义一个简单的训练循环（这里我们直接用我们上一节实现的 `SimpleSGD`）：

```{python}
def run_sgd(theta_init: Tensor, lr: float, steps: int) -> list[Tensor]:
    theta = theta_init.clone().requires_grad_()
    optimizer = dopt.SimpleSGD([theta], lr=lr)
...
```{python}
theta = torch.tensor([-4.0, 1.0])
history = run_sgd(theta, lr=0.035, steps=40)
history = torch.stack(history)

x = torch.linspace(-4.5, 4.5, 200)
y = torch.linspace(-1.5, 1.5, 200)
...
从图中可以看到，参数不是平滑地向最低点移动，而是在两侧陡峭的山坡上来回摆动。这种摆动会浪费很多更新步数（实际上，如果你把学习率改成 0.04，甚至不会收敛！）。
...
这和前面的写法只是符号约定不同。前一种写法把 $v_t$ 看成梯度的累计，参数更新时减去 $\eta v_t$；后一种写法把 $v_t$ 直接看成参数更新量，所以参数更新时加上 $v_t$。

不管怎么写，核心思想都是：

> **Momentum 会把过去的梯度方向累积起来，用一个更平滑、更有惯性的方向更新参数。**

## 4.2.4 用代码实现 Momentum
...
```{python}
def run_momentum(
    theta_init: Tensor, lr: float, momentum: float, steps: int,
) -> list[Tensor]:
    theta = theta_init.clone().requires_grad_()
    optimizer = SGDWithMomentum([theta], lr=lr, momentum=momentum)
    history = [theta.clone().detach()]
...
::: {.callout-note}
注意，当设置 momentum 时，学习率通常需要调小一些。以 `momentum=0.9` 为例，学习率通常需要比普通 SGD 小一个数量级，因此我们这里把 SGD + momentum 的学习率设置为 0.0035，而普通 SGD 的学习率设置为 0.035。
:::

```{python}
theta = torch.tensor([-4.0, 1.0])
sgd_history = run_sgd(theta, lr=0.035, steps=40)
momentum_history = run_momentum(theta, lr=0.0035, momentum=0.9, steps=40)
sgd_history = torch.stack(sgd_history)
momentum_history = torch.stack(momentum_history)
...
ax.set_title('SGD vs Momentum')
plt.show()
```

Momentum 的轨迹通常会更有连续性。它不会像普通 SGD 那样完全由当前梯度决定，而是带着过去的方向继续前进。

当然，momentum 并不意味着一定每次都更稳定。如果学习率太大，或者 $\beta$ 太接近 1，惯性也可能过强，导致参数冲过低点并产生更大的震荡。所以 momentum 不是取消了学习率调参，而是改变了梯度更新方向的方式。
...
普通 SGD 只需要当前参数和当前梯度，而 momentum 还需要记住过去累积的速度。因此，在保存 checkpoint 时，不能只保存模型参数，也应该保存 optimizer 的状态。否则恢复训练后，momentum buffer 会丢失，训练轨迹会发生变化。
...
这里的 $\tilde{\theta}_t$ 就是一个“提前看”的位置。然后，我们在这个“提前看”的位置计算梯度 $g_t$，再更新动量 $v_t$ 和参数 $\theta_{t+1}$。

不过，在深度学习实践中，Nesterov momentum 不一定总是明显优于普通 momentum。它通常是一个可选改进，具体效果仍然依赖任务、模型和超参数。
...
需要注意的是，PyTorch 要求使用 Nesterov momentum 时必须 `momentum` 必须为正，并且 `dampening` 一般要为 0。对初学者来说，不需要一开始纠结这些实现细节，只要记住 `nesterov=True` 表示使用 Nesterov 风格的动量修正。

## 4.2.7 Momentum 的作用和局限
