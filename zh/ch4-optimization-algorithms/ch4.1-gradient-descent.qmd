...
自动微分可以帮我们计算梯度。对于参数 $\theta$，梯度 $\nabla_\theta L(\theta)$ 告诉我们：如果稍微改变参数，损失会如何变化。

但是，知道梯度还不等于知道完整的训练过程。梯度只是告诉我们局部方向，真正训练模型时，我们还需要决定：

1. 参数应该沿着哪个方向移动；
2. 每次应该移动多远；
3. 应该用多少数据来估计这个方向。

这一节我们先从最基础的梯度下降开始，看清楚“根据梯度更新参数”这件事的基本逻辑。然后我们会进一步讨论，为什么在深度学习里更常见的不是完整的梯度下降，而是随机梯度下降，也就是 **SGD（Stochastic Gradient Descent）**。
...
如果梯度是正的，说明当 $\theta$ 增大时，损失会增大。为了让损失变小，我们应该让 $\theta$ 往小的方向走。

如果梯度是负的，说明当 $\theta$ 增大时，损失会减小。为了让损失变小，我们应该让 $\theta$ 往大的方向走。
...
如果学习率太小，每一步走得很谨慎，训练会很慢。损失虽然可能稳定下降，但需要很多次迭代才能接近较好的参数。

如果学习率太大，每一步可能跨得太远，甚至直接越过低点。更严重时，参数会在损失曲面上来回震荡，损失不降反升，训练变得不稳定。

因此，学习率不是一个无关紧要的细节，而是优化过程中最重要的超参数之一。

可以把学习率理解成模型对梯度的信任程度。学习率越大，模型越激进，相信当前梯度方向可以走很远；学习率越小，模型越保守，只沿着当前方向挪动一点点。

在后面的章节里，我们会看到，很多优化器的核心思想其实都和学习率有关。比如 AdaGrad、RMSProp 和 Adam 会为不同参数自动调整有效学习率；学习率调度器则会让全局学习率在训练过程中发生变化。但在最基础的梯度下降里，学习率就是一个固定的手动设置的数。
...
这里的 `x` 和 `y` 就是一个 mini-batch。`loss.backward()` 计算的是当前 mini-batch 上的梯度，`optimizer.step()` 则根据这些梯度更新参数。
