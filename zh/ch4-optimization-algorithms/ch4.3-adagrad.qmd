...
Adagrad 的全称是 **Adaptive Gradient Algorithm**。它的核心是为每个参数维护一个历史梯度平方和。
...
Adagrad 会根据每个方向过去的梯度大小调整步长。$y$ 方向梯度经常较大，所以它的有效学习率会更快变小；$x$ 方向梯度较小，所以它会保留相对更大的有效学习率。
...
def run_adagrad(
    ...
    steps: int,
    eps: float = 1e-10,
) -> tuple[list[Tensor], list[list[float]]]:
...
sgd_history = dopt.run_sgd(loss_fn, theta, lr=0.45, steps=40)
adagrad_history, adagrad_lr_history = run_adagrad(loss_fn, theta, lr=0.45, steps=40)
sgd_history = torch.stack(sgd_history)
adagrad_history = torch.stack(adagrad_history)

x = torch.linspace(-5.5, 5.5, 200)
y = torch.linspace(-2.5, 2.5, 200)
...
ax.set_title('SGD and Adagrad on an Anisotropic Loss')
plt.show()
```

可以看到，SGD 的轨迹在 $y$ 方向震荡较大，而 Adagrad 的轨迹更快地收敛到最优点。这是因为 Adagrad 自动降低了 $y$ 方向的有效学习率，让更新变得更谨慎。
...
```{python}
adagrad_lr_history = torch.stack(adagrad_lr_history)
steps = torch.arange(1, adagrad_lr_history.shape[0] + 1)

fig = plt.figure(2)
ax = fig.add_subplot(1, 1, 1)
ax.plot(steps, adagrad_lr_history[:, 0], marker='o', markersize=3)
ax.plot(steps, adagrad_lr_history[:, 1], marker='o', markersize=3)
ax.set_xlabel('Step')
