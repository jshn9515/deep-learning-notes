...
y = 3 * x - 1 + 0.3 * x.relu() + torch.randn_like(x)

adagrad_losses = train_with_optimizer(dopt.Adagrad, x, y, lr=0.5)
rmsprop_losses = train_with_optimizer(RMSprop, x, y, lr=0.05)
adadelta_losses = train_with_optimizer(Adadelta, x, y, lr=1.5)

fig = plt.figure(3, figsize=(6, 4))
ax = fig.add_subplot(1, 1, 1)
ax.plot(adagrad_losses)
ax.plot(rmsprop_losses)
ax.plot(adadelta_losses)
ax.set_xlabel('Step')
ax.set_ylabel('Loss')
ax.legend(['Adagrad', 'RMSprop', 'Adadelta'])
ax.set_title('Adagrad, RMSprop, and Adadelta')
plt.show()
```
