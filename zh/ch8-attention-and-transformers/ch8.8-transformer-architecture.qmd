...
## 8.8.7 PyTorch 中的 `nn.Transformer`

PyTorch 提供了完整的 Encoder-Decoder Transformer 模块：
...
需要注意的是，`nn.Transformer` 接收的不是 token id，而是已经 embedding 后的向量。
...
下面写一个简化版 Seq2Seq Transformer。这里直接使用 `nn.Transformer`，重点放在完整调用流程，而不是重复实现 Encoder Block 和 Decoder Block。

```{python}
from dnnl.ch8 import SinusoidalPositionalEncoding
...
def generate_causal_mask(seq_len: int, device: Device = None) -> Tensor:
    mask = torch.ones(seq_len, seq_len, device=device)
    # Upper triangular part is 1, lower triangular part is 0
    mask = mask.triu(diagonal=1)
    mask = mask.masked_fill(mask == 1, -math.inf)
    return mask
