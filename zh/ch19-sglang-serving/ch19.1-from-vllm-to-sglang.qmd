...
open-graph:
  title: ch19.1-from-vllm-to-sglang
  description: "19.1 From vLLM to SGLang: Same Serving Problems, Different Designs"
title: "19.1 From vLLM to SGLang：相同的 Serving 问题，不同的设计"
order: 1
---
...
![图 19.1.1 vLLM 的推理引擎](figures/ch19.1-vllm-engine.svg){height=400px}
...
SGLang 的一个代表性设计就是 **RadixAttention** [@zheng2024SGLang]。它不只关心一个请求的 KV cache 放在哪些 page，还会维护不同请求之间的共享前缀关系，使已经计算过的 prefix KV cache 可以被后续请求复用。
...
![图 19.1.4 两个请求可以共享前缀](figures/ch19.1-predix-cache.svg){width=60%}
...
![图 19.1.5 SGLang Runtime 的请求路径](figures/ch19.1-sglang-runtime.png)
...
::: {.list-table tbl-colwidths="[20,40,40]"}
...
- - Prefix Reuse
  ...
  - Radix Cache / RadixAttention

- - Scheduler
  - 根据 token budget、可用 KV Cache block 等构造执行批次
  - 在计算预算和 KV Cache 容量之外，还可以进一步利用 Prefix Cache 信息

- - Runtime 重点
  - Engine、Scheduler、KV Cache Manager、Model Runner
  - TokenizerManager、Scheduler、Radix Cache、ModelRunner、DetokenizerManager
:::
