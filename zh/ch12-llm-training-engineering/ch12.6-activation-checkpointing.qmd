...
open-graph:
  title: ch12.6-activation-checkpointing
  description: "12.6 Activation Checkpointing: Trading Compute for Memory"
title: "12.6 Activation Checkpointing：用重计算换显存"
order: 6
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch12-llm-training-engineering/ch12.6-activation-checkpointing.ipynb){fig-align="left"}
...
## 12.6.1 Backward 为什么需要保存 Activation
...
![图 12.6.1 正常训练中的 activation](figures/ch12.6-activation.svg){width=70%}
...
## 12.6.2 Activation Checkpointing 在保存什么

假设一个模型由四个 block 组成。正常训练时，每个 block 内部为了 backward 可能保存很多 internal activation：

![图 12.6.2.2 正常训练过程](figures/ch12.6-forward-normal.svg){width=70%}
...
![图 12.6.2.3 开启 Activation Checkpoint 过程](figures/ch12.6-forward-checkpoint.svg){height=450px}

这里的 `checkpoint` 和 12.10 要讲的模型 checkpoint 不是一回事。
...
## 12.6.3 Backward 时发生了什么：重新做一次 Forward
...
![图 12.6.3 Backward 时的重计算](figures/ch12.6-backward-checkpoint.svg){height=800px}
...
不过这只是帮助理解的近似。真实 wall-clock time 还会受到 kernel efficiency、memory bandwidth、checkpoint 粒度以及具体算子实现影响，所以不能看到 checkpointing 就直接假设训练一定慢 33%。真正的代价应该像 12.3 一样通过 profiling 和 benchmark 测出来。

## 12.6.4 PyTorch 里最基本的 Checkpoint
...
## 12.6.5 实测 Peak Memory：显存到底能省多少

概念上知道 checkpointing 会减少 activation 还不够，我们可以直接在 GPU 上测量 peak allocated memory。

下面使用刚才的 `MLPBlockStack`，分别运行 `act_ckpt=False` 和 `act_ckpt=True`，然后比较显存占用。同时，我们使用 12.3 讲过的 `torch.Event` 测量平均 step time，看看显存下降的同时付出了多少额外计算时间。
...
如果想进一步知道**哪一段**的 activation 被省掉，可以结合 12.3 的 profiler 和 memory profiling 工具继续分析，而不是只看最终一个 peak memory 数字。

## 12.6.6 Checkpoint 粒度：不是越细越好
...
## 12.6.7 Dropout、随机性和有状态 Forward
...
## 12.6.8 为什么推荐 use_reentrant 等于 False
...
## 12.6.9 Activation Checkpointing 的代价与使用边界
...
![图 12.6.10 Activation Checkpointing 与 Gradient Accumulation](figures/ch12.6-reduce-activation-mem.svg){width=60%}
