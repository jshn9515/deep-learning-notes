...
open-graph:
  title: ch12.10-large-model-checkpoint
  description: "12.10 Large-Model Checkpointing: Restoring DDP and FSDP Training State"
title: "12.10 大模型 Checkpoint：模型、优化器与分布式状态如何恢复"
order: 10
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch12-llm-training-engineering/ch12.10-large-model-checkpoint.ipynb){fig-align="left"}
...
## 12.10.2 DDP Checkpoint：模型是 Replicated 的
...
## 12.10.3 DDP Resume：恢复以后为什么还能保持一致

DDP 的恢复流程通常是：

![图 12.10.3 DDP checkpoint 恢复流程](figures/ch12.10-ddp-resume.svg){height=550px}
...
## 12.10.4 为什么 FSDP Checkpoint 完全不同
...
![图 12.10.4 FSDP checkpoint gather](figures/ch12.10-full-gather-ckpt.svg){height=330px}
...
## 12.10.5 Distributed Checkpoint：不再把 Shard 当普通文件
...
![图 12.10.5 DCP checkpoint 保存和加载流程](figures/ch12.10-dcp.svg){height=430px}
...
## 12.10.7 Stateful：统一 Model 和 Optimizer 的分布式表示
...
## 12.10.8 DCP Save：每个 Rank 到底做了什么

把 `dcp.save()` 再拆细一点，大致可以理解成：

![图 12.10.8 DCP 保存流程](figures/ch12.10-dcp-save.svg){height=580px}
...
## 12.10.9 DCP Load-Time Resharding：从旧布局恢复到新布局
...
![图 12.10.9 DCP 加载流程](figures/ch12.10-dcp-load.svg){height=620px}
...
::: {.list-table}
表 12.10.10 DDP 和 FSDP checkpoint 对比
