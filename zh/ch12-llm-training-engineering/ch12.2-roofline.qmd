...
open-graph:
  title: ch12.2-roofline
  description: "12.2 FLOPs, Memory, and Arithmetic Intensity: Why Compute Is Not the Whole Story"
title: "12.2 FLOPs、Memory 与 Arithmetic Intensity：为什么不是只看计算量"
order: 2
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch12-llm-training-engineering/ch12.2-roofline.ipynb){fig-align="left"}
...
![图 12.2.0 数据搬运过程](figures/ch12.2-data-movement.svg){height=320px}
...
## 12.2.2 Memory Traffic：数据搬运本身也要时间
...
## 12.2.3 Arithmetic Intensity：每搬一个 Byte 做多少计算
...
## 12.2.4 Roofline Model：Compute-Bound 还是 Memory-Bound
...
## 12.2.5 GEMM：为什么矩阵乘法通常能充分利用计算单元
...
## 12.2.7 Transformer 里的算子 Roofline 分析

现在可以把 Transformer 中常见的 operator 放到同一个视角下理解。

::: {.list-table}
表 12.2.7 Transformer 常见算子 Roofline 分析
...
## 12.2.8 Shape 与 Precision 都会改变实际效率
...
## 12.2.9 理论 FLOPs 很重要，但最终要看 Performance
...
1. 先理解 FLOPs、memory traffic 和 arithmetic intensity；
2. 对可能的瓶颈形成假设；
3. 通过 profiling 找到真正占用时间的部分；
4. 再决定怎么优化。
