...
open-graph:
  title: ch12.9-distributed-training
  description: "12.9 Distributed Training: Intuition for DDP, ZeRO, and FSDP"
title: "12.9 分布式训练入门：DDP、ZeRO 与 FSDP 的直觉"
order: 9
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch12-llm-training-engineering/ch12.9-distributed-training.ipynb){fig-align="left"}
...
## 12.9.2 Rank、World Size 与 Process Group
...
## 12.9.3 DistributedDataParallel：每个 Rank 一份完整模型

PyTorch 里最常见的数据并行实现是 `DistributedDataParallel`，通常简称 **DDP**。

DDP 的基本结构可以画成：

![图 12.9.3 DDP 模型结构](figures/ch12.9-ddp.svg){width=80%}
...
![图 12.9.4 DDP backward 中的 overlap](figures/ch12.9-ddp-overlap.svg)
...
## 12.9.5 ZeRO：为什么每张 GPU 都要保存同样的状态
...
## 12.9.6 ZeRO-3：参数平时是 Shard，用到时再 Gather
...
![图 12.9.6.1 reduce-scatter 示意图](figures/ch12.9-reduce-scatter.svg){height=180px}

于是，一整个 block 可以粗略画成：

![图 12.9.6.2 ZeRO-3 中 block 的计算流程](figures/ch12.9-block-life-cycle.svg){height=300px}
...
::: {.list-table}
表 12.9.7 ZeRO 与 PyTorch 分布式 API 的基本对应关系
...
## 12.9.8 FSDP1：用 Wrapper 把模型变成 Fully Sharded
...
把 12.9.2 的 process-group 初始化也放进来，一个最小的 FSDP1 结构可以写成：
...
当使用 `ShardingStrategy.FULL_SHARD` 时，FSDP1 会对 parameters、gradients 和 optimizer states 进行分片。对于一个 FSDP unit，具体计算过程和图 12.9.6.2 一致。因此，FSDP1 的 `FULL_SHARD` 基本就是前面介绍的 ZeRO-3 风格。
...
## 12.9.9 FSDP2：fully_shard() 代替 FSDP1 Wrapper
...
## 12.9.10 从 All-Reduce 看 DDP、ZeRO 与 FSDP
...
![图 12.9.10 ZeRO 的渐进式分片](figures/ch12.9-ddp-zero-fsdp.svg){height=360px}

可以用下面这张表建立整体直觉：

::: {.list-table}
表 12.9.10 DDP、ZeRO 与 PyTorch FSDP 的对比
...
这正好和 12.5 的 accumulation window 对齐。
...
![图 12.9.12 选择 DDP / ZeRO / FSDP 的直觉流程](figures/ch12.9-summary.svg)
