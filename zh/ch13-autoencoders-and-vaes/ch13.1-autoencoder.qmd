...
[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch13-autoencoders-and-vaes/ch13.1-autoencoder.ipynb){fig-align="left"}
...
![图 13.1.1：AutoEncoder 网络结构 [@Mathworks2026AutoEncoder]](figures/ch13.1-autoencoder.svg){height=225px}
...
这里我们定义了一个基本的 `AutoEncoder` 类，包含一个 encoder 和一个 decoder。编码器是一个两层的全连接网络，输入维度是 784（$28\times 28$ 的图像展平），输出维度是 32（latent space 的大小）。解码器也是一个两层的全连接网络，把 latent 向量从 32 维还原回 784 维。
...
可以看到，虽然重建的图像可能有些模糊，但整体轮廓和数字形状都能被还原出来。这说明 AutoEncoder 确实学到了输入数据的某种有效表示，而不是简单地记住每个像素值。
...
这就把我们带到了下一节：**Variational Autoencoder (VAE)** [@kingma2022VAE]。
