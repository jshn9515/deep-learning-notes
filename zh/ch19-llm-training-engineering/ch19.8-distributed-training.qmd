...
for epoch in range(num_epochs):
    # Must happen before go through the dataloader
...
PyTorch 里也能看到这几个 stage 对应的实现。最简单的是 `ZeroRedundancyOptimizer`。它和 DDP 配合使用，只把 optimizer states 分散到不同 rank，因此可以把它理解成 **ZeRO-1 风格**的实现：
...
1. 平时：每个 rank 只保留自己的参数 shard；
2. Forward 前：通过 all-gather 把完整参数恢复出来；
3. Forward 后：把完整参数释放，只保留自己的 shard；
4. Backward 前：再次 all-gather 完整参数；
5. Backward 后：reduce-scatter 梯度，只保留自己的 gradient shard。

这里的 reduce-scatter 是一个非常重要的 collective。它和 all-gather 正好相反：每个 rank 提供一份数据，先做 reduction，然后把结果分片给每个 rank：

![图 19.8.6.1 reduce-scatter 示意图](figures/ch19.8-reduce-scatter.svg){height=180px}

于是，一整个 block 可以粗略画成：

![图 19.8.6.2 ZeRO-3 中 block 的计算流程](figures/ch19.8-block-life-cycle.svg){height=300px}
...
## 19.8.7 从 ZeRO 到 PyTorch FSDP

前面讲的 ZeRO 更像是一套**训练状态如何分片**的思想：先分 optimizer states（ZeRO-1），再分 gradients（ZeRO-2），最后连 parameters 也一起分（ZeRO-3）。

PyTorch 自己也提供了对应的实现。把最基础的关系放在一起，可以先记成：

::: {.list-table}
表 19.8.7 ZeRO 与 PyTorch 分布式 API 的基本对应关系
...
这里需要注意，ZeRO 和 FSDP 不是两套完全无关的东西。ZeRO 描述的是：哪些训练状态应该 replicated，哪些训练状态应该 sharded；FSDP 描述的是：PyTorch 如何把 fully sharded data parallel 真正放进 module、autograd、collective communication 和 optimizer 的训练流程里。

对于 FSDP，PyTorch 先后提供了两代主要接口：FSDP1 和 FSDP2。下面分别看一下它们最基本的写法。

## 19.8.8 FSDP1：用 Wrapper 把模型变成 Fully Sharded Data Parallel
...
把 19.8.2 的 process-group 初始化也放进来，一个最小的 FSDP1 结构可以写成：
...
model = FullyShardedDataParallel(
    ...
    auto_wrap_policy=wrap.transformer_auto_wrap_policy(...),
...
注意，这里的 optimizer 不需要使用 `ZeroRedundancyOptimizer`，因为 FSDP1 会在内部把 optimizer states 分片。
...
当使用 `ShardingStrategy.FULL_SHARD` 时，FSDP1 会对 parameters、gradients 和 optimizer states 进行分片。对于一个 FSDP unit，具体计算过程和图 19.8.6.2 一致。因此，FSDP1 的 `FULL_SHARD` 基本就是前面介绍的 ZeRO-3 风格。
...
## 19.8.9 FSDP2：fully_shard() 代替 FSDP1 Wrapper
...
那么 forward 结束以后不会马上 reshard。因为 full parameter 还在，所以 backward 不需要再次 all-gather。因此可以把它理解成 **ZeRO-2 风格**的 FSDP2 执行方式：gradients 和 optimizer states 被 shard，而 forward materialize 出来的完整 parameters 会一直保留到 backward 使用完。
...
- FSDP1 `FULL_SHARD` 对应 FSDP2 `reshard_after_forward=True`：ZeRO-3 风格，forward 后立即 reshard，backward 前需要 all-gather；
- FSDP1 `SHARD_GRAD_OP` 对应 FSDP2 `reshard_after_forward=False`：ZeRO-2 风格，forward 后保留 full parameters，backward 前少一次 all-gather。

这也说明 `reshard_after_forward` 本质上仍然是在做前面反复出现的 trade-off：

> **是否愿意多占一些显存，换取更少的通信。**

## 19.8.10 从 All-Reduce 看 DDP、ZeRO 与 FSDP
...
![图 19.8.10 ZeRO 的渐进式分片](figures/ch19.8-ddp-zero-fsdp.svg){height=360px}

可以用下面这张表建立整体直觉：

::: {.list-table}
表 19.8.10 DDP、ZeRO 与 PyTorch FSDP 的对比
...
- - ZeRO-1 / `ZeroRedundancyOptimizer`
  - Full
  - Full
  - Shard
...
- - FSDP2 `reshard_after_forward=False`
  - Forward 后暂时 Full
  - Shard
  - Shard
  - Forward All-Gather + Gradient Reduce-Scatter

- - FSDP2 `reshard_after_forward=True`
  - 不计算时 Shard
  - Shard
  - Shard
...
## 19.8.11 和 Gradient Accumulation、Activation Checkpointing 怎么组合
...
如果 parameters 和 gradients 都能放下，但 Adam optimizer states 已经成为明显的显存压力，可以先想到 ZeRO-1。PyTorch 的 `ZeroRedundancyOptimizer` 就体现了这个思路：parameters 和 gradients 仍然 replicated，只把 optimizer states 分散到不同 rank。

如果还希望继续减少 gradient memory，可以进一步进入 ZeRO-2 风格。在 PyTorch FSDP 中，可以把 FSDP1 的 `SHARD_GRAD_OP`，或者 FSDP2 的：
...
如果连完整 parameters 都不希望长期留在每个 rank 上，就进入 fully sharded，也就是 ZeRO-3 风格。对应到 PyTorch，就是 FSDP1 的 `FULL_SHARD`，或者 FSDP2 的 `reshard_after_forward=True`。

我们可以把选择流程画成：

![图 19.8.12 选择 DDP / ZeRO / FSDP 的直觉流程](figures/ch19.8-summary.svg)
...
到这里，我们已经把大模型训练里的 memory、profiling、mixed precision、gradient accumulation、activation checkpointing 和 distributed training 串了起来。下一节会从训练系统回到更底层的执行效率：如何在 GPU 上高效执行 attention，以及 PyTorch 和 Transformers 常见的 attention 算子。
