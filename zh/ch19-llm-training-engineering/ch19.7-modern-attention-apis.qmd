...
open-graph:
  title: ch19.7-modern-attention-apis
  description: "19.7 Modern Attention APIs and Hugging Face Kernels"
title: "19.7 现代 Attention API 与 Hugging Face Kernels"
order: 7
---

[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jshn9515/dnnl-notebooks/blob/main/zh/ch19-llm-training-engineering/ch19.7-modern-attention-apis.ipynb){fig-align="left"}
...
## 19.7.1 SDPA：PyTorch 的统一 Attention 入口
...
## 19.7.2 SDPA 为什么比手写 Attention 更适合作为默认实现
...
## 19.7.3 torch.nn.attention：控制 SDPA 行为的命名空间
...
### 19.7.3.1 SDPBackend：SDPA 有哪些后端
...
def benchmark_sdpa(
    ...
    with attn.sdpa_kernel(attn_backend):
        ...
        accl.synchronize()  # Make sure all warmup kernels are finished
...
## 19.7.4 FlexAttention：当 SDPA 的 Mask 不够灵活
...
## 19.7.5 xFormers：PyTorch 原生 SDPA 之前的重要高效算子库
...
```code
xFormers output shape: torch.Size([16, 4, 128, 32])
```
...
## 19.7.6 Hugging Face kernels：从 Hub 动态加载 kernel
...
下一节，我们将进一步下沉到 GPU kernel，学习 Triton 的基本执行模型，以及什么时候值得自己写 fused kernel。现代 Attention API 帮我们优先复用成熟的高性能实现，而 Triton 要回答的是：

> **当现有算子和编译器仍然无法消除瓶颈时，如何获得更细粒度的 GPU 控制。**
