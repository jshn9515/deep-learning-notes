...
open-graph:
  title: ch19.10-triton
  description: "19.10 Triton: When Do We Need to Write Our Own Kernel?"
title: "19.10 Triton 入门：什么时候需要自己写 Kernel"
order: 10
---
...
## 19.10.1 为什么 PyTorch 代码不一定等于一个 kernel
...
![图 19.10.1.1 PyTorch Eager Expression](figures/ch19.10-tensor-op.svg)
...
![图 19.10.1.2 Fused Kernel](figures/ch19.10-fused-kernel.svg){height=260px}
...
## 19.10.2 Triton 的基本执行模型：一个 Program 处理一块数据
...
::: {.list-table}
表 19.10.2 CUDA 与 Triton 的对应关系表
...
## 19.10.3 第一个 Triton Kernel：Vector Addition
...
![图 19.10.3 Triton 的执行顺序](figures/ch19.10-triton-execution.svg){height=450px}

代码里的 `@triton.jit` 表示这个函数会由 Triton JIT compiler 编译成 GPU kernel；`a_ptr`、`b_ptr` 和 `y_ptr` 可以理解为指向 GPU tensor 数据的 tl.tensor。`tl.load()` 根据 tl.tensor 和 offsets 读取一整个 block，计算完成后，再通过 `tl.store()` 写回结果。

因此，Triton 代码虽然写起来像 Python，但 kernel 内部并不是普通 Python 在逐元素执行。

## 19.10.4 mask 为什么几乎到处都会出现
...
## 19.10.6 从多个 Operator 到一个 Kernel：Fused SwiGLU
...
![图 19.10.6.1 PyTorch Eager Expression](figures/ch19.10-swiglu-eager.svg){height=550px}

而 Triton fused kernel 可以把中间结果保留在当前 program 的计算过程中：

![图 19.10.6.2 Triton Fused Kernel](figures/ch19.10-swiglu-fused.svg){height=260px}
...
另外，benchmark 不能只测试一个 shape。可能一个 kernel 在 `[4096, 4096]` 上上表现很好，但不代表在 `[8, 4096]` 上仍然很好。LLM 中的 batch size、sequence length、hidden size 都可能改变 kernel 的工作规模，因此最终应该测试真实 workload 中常见的 shape 分布，而不是只选择一个 benchmark。

## 19.10.9 为什么训练 Kernel 比 Forward Demo 更难
...
![图 19.10.10 Triton Fusion Judgement](figures/ch19.10-triton-judgement.svg){height=550px}
...
这一章，我们从显存账、计算与访存、profiling、mixed precision，一直到 gradient accumulation、activation checkpointing、distributed training 和 Triton，看起来讨论了很多不同技术，但它们解决的其实是同一个工程问题：

> **先找到真正受限的资源，再选择对应的优化方法。**

显存不够时，我们可以减少 activation、累积 gradient 或拆分 model state；计算精度过高时，可以使用 mixed precision；通信成为瓶颈时，需要改变 distributed strategy；而当 profiler 最终把问题定位到某个具体 kernel 时，Triton 才真正进入我们的工具箱。

这也是性能优化里最重要的顺序：**先测性能，再做优化。**
