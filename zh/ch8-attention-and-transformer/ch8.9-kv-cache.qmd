...
title: "8.9 KV Cache：为什么推理时不用重复算过去"
---

前面几节里，我们已经看到，Transformer Decoder 在推理时通常是**自回归生成**的。也就是说，模型不是一次性生成完整句子，而是一步一步生成：
...
- 当前新 token 的 query：表示“我现在想找什么”；
- 过去 token 的 key：表示“过去每个位置怎么被匹配”；
- 过去 token 的 value：表示“过去每个位置能提供什么内容”。
...
这里的 $K_{\le t}$ 和 $V_{\le t}$ 包含从第 1 个位置到第 $t$ 个位置的 key 和 value。

注意一个重要事实：过去位置的 key 和 value 一旦算出来，在后续生成过程中就可以复用。

比如 $k_1$ 和 $v_1$ 在生成第 2 个 token 时用过，在生成第 3 个、第 4 个、第 5 个 token 时还会继续被用到。但它们本身不会因为后面生成了新 token 而改变。因此，我们可以把过去的 key 和 value 存起来。下一步推理时，只需要计算新 token 对应的 key 和 value，再把它们追加到缓存里。

这就是 **KV Cache**。

至于 query，通常不需要缓存。因为在自回归推理中，每一步真正要输出的是最新位置的结果。过去位置的 query 已经用来计算过过去位置的输出了，后续不会再用它们重新生成过去 token。

所以，推理时缓存的是 $K_{\text{cache}},\quad V_{\text{cache}}$，而不是 $Q_{\text{cache}}$。
...
这意味着第 1 个 token 的 key 和 value 会被重复计算很多次。从计算量上看，生成长度为 $T$ 的序列时：
...
也就是说，整体会随生成长度近似二次增长。当然，这里只是一个直观估计。真实 Transformer 里还有多层、多头、矩阵乘法和具体实现细节。但核心问题不变：如果每一步都重新处理完整前缀，就会产生大量重复计算。

## 8.9.4 有 KV Cache 时发生了什么

有了 KV Cache 以后，推理过程会变成下面这样。

第一步，输入第一个 token，计算它的 key 和 value：
...
$$
K_{\text{cache}} = [k_1],\quad V_{\text{cache}} = [v_1]
$$

第二步，输入第二个 token，只计算新 token 的 $q_2, k_2, v_2$，然后把新的 key 和 value 追加到缓存里：

$$
K_{\text{cache}} = [k_1, k_2],\quad V_{\text{cache}} = [v_1, v_2]
$$

接着，用当前 query $q_2$ 去看缓存中的所有 key/value：

$$
\operatorname{softmax} \left(\frac{q_2 K_{\text{cache}}^\top}{\sqrt{d_k}} \right) V_{\text{cache}}
$$

第三步也是一样。只计算最新 token 的 $q_3, k_3, v_3$，然后追加：

$$
K_{\text{cache}} = [k_1, k_2, k_3],\quad V_{\text{cache}} = [v_1, v_2, v_3]
$$

再用 $q_3$ 去看所有历史 key/value。这样一来，每一步都不需要重新计算过去 token 的 key 和 value。过去的信息已经存好了，直接拿来用就可以。

## 8.9.5 KV Cache 改变的是推理，不是训练

这里要特别注意：

> **KV Cache 主要用于自回归推理，而不是普通训练。**

训练时，我们通常已经知道完整目标序列。比如机器翻译任务中，目标句子是：

```text
I love deep learning.
```

训练时我们仍然可以一次性把整个目标序列输入 Decoder，并用 causal mask 保证每个位置只能看到自己之前的位置。也就是说，训练时虽然不能偷看未来，但计算上仍然可以并行处理整个序列。这时候没有必要像推理那样一步一步生成，也就不需要 KV Cache 来避免前缀重复计算。
...
class SelfAttentionWithKVCache(nn.Module):
    def __init__(
        ...
        self.in_dim = in_dim or attn_dim
        self.out_dim = out_dim or attn_dim

        self.q_proj = nn.Linear(self.in_dim, attn_dim)
        self.k_proj = nn.Linear(self.in_dim, attn_dim)
        self.v_proj = nn.Linear(self.in_dim, attn_dim)
...
attention = SelfAttentionWithKVCache(attn_dim=d_model)
...
这个例子里，`past_k` 和 `past_v` 就是缓存。第一次调用时，cache 为空，模型计算当前 token 的 key/value，并返回它们；第二次调用时，模型只计算新 token 的 key/value，然后和过去缓存的 key/value 拼接起来。
...
## 8.9.8 从复杂度角度理解 KV Cache

最后，我们再用一个更直观的方式总结 KV Cache 的作用。

假设已经生成了 $t$ 个 token。没有 KV Cache 时，第 $t$ 步需要重新处理长度为 $t$ 的前缀。模型不仅要计算新 token 的表示，还要重新计算所有历史 token 的 key/value 和中间状态。有 KV Cache 时，第 $t$ 步只需要处理最新的 1 个 token，同时让当前 token 去查询长度为 $t$ 的 KV 缓存。

也就是说，KV Cache 并不会让 attention 完全变成常数时间。因为当前 query 仍然要和所有历史 key 做匹配：

$$
q_t K_{\le t}^\top
$$

这个计算仍然会随着上下文长度增长。但它避免了更大的浪费：重复计算所有历史 token 在每一层里的 key/value。所以更准确地说，KV Cache 的作用不是让生成成本和长度无关，而是把每一步重新跑完整前缀变成每一步只处理新增 token，并读取历史缓存。这对大模型推理非常重要。
