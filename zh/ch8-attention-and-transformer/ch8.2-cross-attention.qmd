...
```{python}
X = torch.randn(4, 4)  # query sequence, length = 4
Y = torch.randn(6, 4)  # key/value sequence, length = 6

# Note that W_Q and W_K must have the same output dimension (3),
# because Q and K are compared by dot product.
# W_V can have a different output dimension (2),
# because it determines the dimension of the final output.
W_Q = torch.randn(4, 3)  # query projection
W_K = torch.randn(4, 3)  # key projection
W_V = torch.randn(4, 2)  # value projection
...
可以看到，attention weights 的形状是 `(query length, key/value length)`。也就是说，查询序列中的每个位置，都会对被查询序列中的所有位置分配一组权重。
...
im = ax.pcolormesh(attn_weights, cmap='Blues', vmin=0, vmax=1)
x_ticks = np.arange(Y.size(0))
y_ticks = np.arange(X.size(0))
...
```{python}
def average_entropy(d_k: int, num_queries: int = 2048, num_keys: int = 10):
